...
- **Demo vivo**: app Vite em `examples/gallery` (40 sections) consome o SDK via `file:../..`.
...
│   ├── icons/       ⇢  <Icon name> por slug + IconProvider + 25 shards gerados (generated/)
...
`FilterBar`/QueryBuilder · `Tour`/coachmarks · `Markdown`. (**`Chat`**, **`Transfer`** e **`Masonry`** saíram da lista — thread + `ChatComposer`, com agrupamento por autor/dia, estado de entrega e a regra de rolagem "só pula se já estava no fim".)
