...
## Ganho acima de 100%: `createAudioBus`

**Quando usar:** você precisa **subir** o volume de uma fonte, misturar várias e mandar tudo para um dispositivo escolhido. Chamada com volume por participante é o caso típico.

`element.volume` é clampado em `1`. Ou seja, um participante que fala baixo só pode ser **abaixado** — a única correção que ninguém precisa. Subir acima de 100% exige um grafo WebAudio, e montá-lo direito esbarra em três coisas não-óbvias, que é o que este barramento embala.
...
const handle = bus.attach(streamRemoto, { gain: 1 });
handle.setGain(2.4); // acima de 1 — o ponto da coisa toda
handle.stop();

bus.setMasterGain(1.2);
await bus.setOutputDevice(idDoFone); // "" = padrão do sistema
bus.close();
```

No React, `useAudioBus` faz o mesmo e fecha o contexto no unmount:

```tsx
import { useAudioBus } from "tempest-react-sdk";
import { useEffect } from "react";

function Participante({ stream, volume }: { stream: MediaStream; volume: number }) {
...
### As três armadilhas que o barramento resolve

!!! danger "O bug do Chrome que deixa o grafo mudo"
    Um `MediaStreamAudioSourceNode` sobre um stream **remoto** de WebRTC não produz amostras no Chrome a menos que o mesmo stream esteja também preso a um elemento de mídia ([crbug.com/687574](https://crbug.com/687574)). O grafo fica visivelmente correto e **completamente silencioso** — sem conhecer o bug, é um dia de depuração. O `attach()` cria um `<audio muted autoplay>` de âncora para você. Ele parece código morto e **não é**: apagá-lo emudece a chamada inteira no Chrome.

!!! warning "O limiter fica depois da soma, nunca por fonte"
    Clipping é propriedade da **mistura**, não do participante: três pessoas a 200% cada ficam limpas sozinhas e distorcem no instante em que falam juntas. Um limiter por participante não enxerga isso; o do master enxerga. Ajuste com `limiter: { threshold: -3 }`, ou desligue com `limiter: false`.

!!! info "`setSinkId` mora no elemento, não no contexto"
    Mandar a chamada para o fone enquanto o resto do sistema fica no alto-falante só é alcançável saindo por `MediaStreamAudioDestinationNode` → `<audio>` → `setSinkId`. `AudioContext.setSinkId` tem suporte bem menor, então a mistura sai por um elemento real. `bus.canSelectOutput` diz se o motor permite: no Safari e em **todo** browser iOS a saída segue a rota do sistema, e o seletor deve ser **escondido**, não oferecido e ignorado.

### Detalhes que evitam surpresa

- **O teto é seu.** `maxGain` (default `DEFAULT_MAX_GAIN`, que é `3`) limita fonte e master. Acima de ~3 a fala já chega distorcida antes de ficar mais alta.
...
- `createAudioBus` / `useAudioBus` — ganho **acima de 100%**, limiter depois da soma e `setSinkId`; `attach()` traz a âncora do crbug.687574 que impede o grafo mudo no Chrome.
