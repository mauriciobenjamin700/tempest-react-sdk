...
| `onReact` | `(message, emoji: string) => void` | — | Liga os chips de reação. Recebe o emoji pressionado. |
| `messageActions` | `(message) => ContextMenuItem[]` | — | Ações por mensagem: clique direito, toque longo e botão `⋮`. |
| `onQuoteClick` | `(message) => void` | — | Clique no stub de resposta — role até a mensagem citada. |
...
### Mensageiro: o que uma bolha carrega além de texto

O `<Chat>` nasceu como thread de texto, e é isso que ele resolve para suporte e comentário de documento. Para um mensageiro entre pessoas ele parava no primeiro balão que não era texto puro — foi construindo o `tempest-zap` que isso ficou concreto: o app escreveu `MessageBubble`, `MessageThread` e `MessageComposer` próprios, não por gosto, mas porque nada disso tinha onde encaixar.
...
const mensagens: ChatMessage[] = [
  {
    ...
    body: "olha a foto do lugar",
    attachments: [{ kind: "image", url: "/fotos/sala.jpg", alt: "sala vazia" }],
  ...
  },
  {
    ...
    quote: { messageId: "m1", senderName: "Ana", excerpt: "olha a foto do lugar", kind: "image" },
...
| --- | --- | --- |
| `attachments` | `ChatAttachment[]` | Imagem (abre no `Lightbox`), vídeo e áudio (players do SDK), nota de voz com waveform, documento como linha de download |
| `quote` | `ChatQuote` | O stub da mensagem respondida, com `revoked` para quando ela é apagada |
| `reactions` | `ChatReaction[]` | Emoji com contagem, marcando a sua |
| `receipt` | `ChatReceipt` | `deliveredTo` / `readBy` / `totalRecipients` — os ticks de um grupo |
| `deleted` | `boolean` | O tombstone, como estado, não como `body` trocado por uma string |
| `edited` | `boolean` | Marca "editada" sem tocar no corpo |

!!! danger "Apagar uma mensagem tem de apagar a citação dela"
    `ChatQuote.revoked` não é enfeite. Sem ele, o texto de uma mensagem apagada continua visível dentro da resposta de quem a citou — o vazamento mais fácil de passar despercebido, porque a mensagem original some da tela e a cópia não. Com `revoked: true` o stub troca o trecho por "Mensagem apagada" e nada do conteúdo sobrevive.

!!! info "Ticks de grupo precisam de três estados, não de dois"
    `status: "read"` é um booleano do remetente: ele não distingue "entregue a todos" de "lida por todos", e só o segundo pinta o tick. Com `receipt`, `resolveReceiptState` devolve `"sent" | "delivered" | "read"` — e os limiares são deliberadamente **todos**: reportar "lida" porque uma pessoa de nove abriu é o que faz o indicador perder credibilidade. Quando `totalRecipients` é `0` (conversa cuja composição o app não acompanha), o `status` volta a mandar.

!!! tip "Ações ficam num menu, não pintadas no balão"
    Botões em cada bolha transformam a conversa numa barra de ferramentas. `messageActions` liga o `<ContextMenu>` do SDK: clique direito no desktop, **toque longo** no celular e um botão `⋮` — que é também o caminho do teclado. Mensagem apagada não recebe ações.

!!! check "Mídia e menu chegam por `lazy()` — a thread de texto não paga"
    Medido no `dist`, empacotado com code splitting: a thread em si sai em **3,67 kB brotli**, o caminho de mídia (`Lightbox` + `VideoPlayer` + `AudioPlayer`) em **4,91 kB** num chunk que só é pedido quando aparece um anexo, e o menu de ações em **1,42 kB** pedido só quando você passa `messageActions`. Um chat de suporte ou uma thread de comentários continuam pagando o tamanho que pagavam.
...
Exportado à parte pelo mesmo motivo do `ChatComposer`: quem monta a própria thread (lista virtualizada, layout de duas colunas) reaproveita a bolha inteira — citação, anexos, reações, ticks, menu de ações e tombstone — em vez de recriá-la. Recebe `message`, `own`, `locale` e os mesmos handlers do `<Chat>`.

Para os ticks fora da bolha existem as duas funções puras que ela usa: `resolveReceiptState(message)` devolve `"sending" | "sent" | "delivered" | "read" | "failed"`, e `receiptLabel(receipt, strings)` devolve a frase ("Lida por 4 de 9") que vai no `title` e no leitor de tela.
