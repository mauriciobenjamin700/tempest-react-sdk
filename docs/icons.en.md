...
### What the table covers, and how it was chosen

`materialToLucide` holds **214 pairs**. Material Symbols publishes ~6100 names,
and the table is the **head of that list by Google's own popularity ranking**
(`fonts.google.com/metadata/icons`) — real usage data rather than a guess at which
names a seed will contain. Every key was checked against that vocabulary and every
target against lucide's canonical slugs; the tests in this folder assert both over
the whole table, so a lucide rename fails in CI instead of in a user's grid.

It is still a seed, not the whole vocabulary: it grows the same way, one
hand-written pair at a time. A map generated by name heuristics gets it badly
wrong, starting with `build`, which is a wrench in Material Symbols and has
nothing to do with construction.

#### Where the two vocabularies do not line up one-to-one

Three patterns, all deliberate:

| Pattern | Example | What happens |
| --- | --- | --- |
| **Outline and filled are one glyph** | `favorite` / `favorite_border`, `check_circle` / `check_circle_outline`, `person` / `person_outline` | In lucide, filled versus outline is a rendering choice on one icon, so the pair lands on the same slug |
| **Variant distinctions collapse** | `build` / `handyman` / `hardware` → `wrench`; `view_list` / `list_alt` / `format_list_bulleted` / `reorder` → `list`; the iOS chevrons → the plain chevron | Material Symbols separates them, lucide does not |
| **A few lose meaning** | see below | Pass an explicit slug where that matters |

The ones that genuinely shift meaning — worth knowing:

| Material Symbol | lucide | What is lost |
| --- | --- | --- |
| `delete_forever` | `trash-2` | The "permanent" |
| `add_shopping_cart` | `shopping-cart` | The "add" |
| `picture_as_pdf` | `file-text` | lucide has no PDF glyph |
| `face` | `face-slightly-smiling` | A neutral face becomes a smiling one |
| `history` | `rotate-ccw-clock` | A clock with an arrow, not a list of past events |
| `payments` | `wallet` | A card stack becomes a wallet |
| `category` | `shapes` | Generic shapes |
| `plumbing` | `shower-head` | lucide has no pipe |

!!! info "Why the 37 identity pairs are in the table"
    Names like `settings`, `code`, `search`, `menu`, `star`, `map` and `folder`
    match in both vocabularies, so they already render today. Leaving them out
    would send them to the fallback and make the bridge a **regression** for
