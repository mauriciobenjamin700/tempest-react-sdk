...
!!! check "Filtro novo começa da página 1"
    Ponha os filtros aplicados na `queryKey` (`["users", filtros]`). Quando ela muda estruturalmente, a página volta para 1 **no mesmo render** — sem a requisição intermediária da página antiga. Até a 0.68.0 a página sobrevivia: filtrar na página 7 pedia a página 7 de um resultado de duas. Para separar rascunho de aplicado, veja [`useDraftFilters`](./hooks.md#rascunho-aplicado-usedraftfilters).
...
### Descobrir qual envelope veio

Endpoint que responde os dois formatos (ou código genérico que aceita
qualquer um) precisa distinguir em runtime. Os type guards fazem isso e já
estreitam o tipo:
...
`isOffsetPage` exige `items` + `total` + `pages`; `isCursorPage` exige `items` +
`has_more` + a chave `next_cursor`. Para o primeiro render antes de qualquer
resposta, `emptyOffsetPage<Item>(25)` devolve um envelope zerado pronto para
`placeholderData`/`initialData` — evita o `items?.map` espalhado pela tela.
...
!!! danger "Onde guardar o token"
    ...
    Cookie tira o token do alcance do JS e o põe no alcance do CSRF: o navegador anexa o cookie a um `POST /auth/refresh` que **outro** site disparou. O par é `csrf: true`, que ecoa o cookie `csrf_token` do `CSRFMiddleware` em `X-CSRF-Token` em toda escrita — login e refresh inclusive:
    ...
    O backend precisa montar o `CSRFMiddleware` e emitir o cookie numa rota `GET` antes da primeira escrita. Ver [CSRF](./http.md#csrf-o-cookie-de-sessao-precisa-de-um-par).
