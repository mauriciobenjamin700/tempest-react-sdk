...
!!! info "`styles.css` is still published"
    One line, all 130 components, and nothing breaks: the bundler deduplicates the
...
The ink a component uses to say **"this one is selected"**: the active segment of `SegmentedControl`, the highlighted row of `Command`, `DropdownMenu` and `ContextMenu`.

It exists because a surface cannot do that job. The four surfaces are one short step apart by design:
...
Not even the extremes of the ramp reach the **3:1** WCAG 2.2 SC 1.4.11 asks of a non-text indicator — and widening the step to make one would ruin every card stacked on a background.

!!! danger "A brand tint does not fix it either — which is why this token is not `--tempest-primary-soft`"
    The intuitive answer is to mark selection with `--tempest-primary-soft`, which is what `ToggleGroup`, `ListTile`, `TreeView` and `NavigationRail` do. Measured across the twelve brands of the theme guard, `--tempest-primary-soft` lands between **1.03:1 and 1.61:1** against `--tempest-bg` in both schemes: it differs in **hue**, and WCAG contrast does not count hue.

    So the indicator is a saturated ink drawn **on top of** the tint — a border on `SegmentedControl`, a left bar in the three menus — rather than a swapped background. The tint stays; it reinforces the signal instead of being it.

A theme generated by `createTheme` emits this token the same way it derives the focus ring: walk the brand ramp until the ink clears 3:1 against all four surfaces, in both schemes. Unlike the ring, it **never** takes an alpha — `focusRingAlpha` does not reach it, because a translucent indicator has the contrast of whatever it composites over, which is the defect this token ends.

```css
.my-app {
    /* opaque, and measured against the four surfaces */
    --tempest-selected-indicator: #7c3aed;
}
```

!!! check "The guard"
    `src/styles/selected-indicator.contrast.test.ts` measures the token against the four surfaces in both themes and **also** pins the numbers above — so if "just use `surface-2`, that is two steps" comes back as a proposal, the assertion that fails it is already written. `src/styles/selected-indicator.usage.test.ts` keeps the four components drawing the state with the token, and every other component off it.
