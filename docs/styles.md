...
!!! warning "Fora de um bundler, use o loader que o pacote ships"
    O import de `.css` que faz isso funcionar é o mesmo que o Node não sabe
    carregar: `node -e "import('tempest-react-sdk')"` morre com
    `ERR_UNKNOWN_FILE_EXTENSION`. Vite, webpack, Rspack, Parcel e esbuild
    resolvem esse import; Node cru, um script `.mjs` solto e o `require()` do
    Jest não. Como o SDK é client-side only e roda sempre atrás de um bundler,
    nenhum consumidor real de app fica de fora — mas o seu script, o seu runner
    de teste e qualquer leitura da superfície por introspecção ficam.

    Para esses, o pacote ships a peça em vez de mandar você escrevê-la:

    ```bash
    node --import tempest-react-sdk/node-css-loader seu-script.mjs
    ```

    Ou de dentro do script, **antes** do primeiro import do pacote:
    ...
    Ele resolve toda folha para um módulo vazio, em `import` **e** em `require`
    ...
    Com `test/css-stub.js` exportando um objeto vazio (`module.exports = {}`).
...
!!! info "`styles.css` continua publicado"
    Uma linha, todos os 130 componentes, e nada quebra: o bundler deduplica a
...
```css
--tempest-selected-indicator: #0052cc; /* dark: #60a5fa */
```

A tinta com que um componente diz **"este aqui está selecionado"**: o segmento ativo do `SegmentedControl`, a linha destacada do `Command`, do `DropdownMenu` e do `ContextMenu`.

Ele existe porque superfície não faz esse trabalho. As quatro superfícies são, de propósito, um degrau curto uma da outra:
...
Nem os extremos da rampa alcançam os **3:1** que a WCAG 2.2 SC 1.4.11 pede de indicador não-textual — e alargar o degrau para servir de indicador estragaria todo cartão empilhado sobre fundo.

!!! danger "Tint da marca também não resolve — e é por isso que este token não é `--tempest-primary-soft`"
    A resposta intuitiva é marcar a seleção com `--tempest-primary-soft`, que é o que `ToggleGroup`, `ListTile`, `TreeView` e `NavigationRail` fazem. Medido nas doze marcas do guard de tema, `--tempest-primary-soft` fica entre **1,03:1 e 1,61:1** contra `--tempest-bg` nos dois esquemas: ele muda de **matiz**, e o contraste WCAG não conta matiz.

    Por isso o indicador é uma tinta saturada desenhada **sobre** o tint — borda no `SegmentedControl`, barra à esquerda nos três menus —, e não a troca do fundo. O tint continua lá; ele é reforço, não o sinal.

Um tema gerado por `createTheme` emite este token pelo mesmo método do anel de foco: sobe a rampa da marca até a tinta alcançar 3:1 contra as quatro superfícies, nos dois esquemas. Diferente do anel, ele **nunca** aceita alfa — `focusRingAlpha` não chega aqui, porque indicador translúcido tem o contraste do que estiver embaixo, que é exatamente o defeito que este token encerra.

```css
.my-app {
    /* opaco, e aferido contra as quatro superfícies */
    --tempest-selected-indicator: #7c3aed;
}
```

!!! check "O guard"
    `src/styles/selected-indicator.contrast.test.ts` mede o token contra as quatro superfícies nos dois temas e **também** fixa os números acima — se alguém propuser de novo "usa `surface-2`, que dá dois degraus", a asserção que reprova a proposta já está escrita. `src/styles/selected-indicator.usage.test.ts` garante que os quatro componentes desenham o estado com o token, e que nenhum outro o usa por engano.
