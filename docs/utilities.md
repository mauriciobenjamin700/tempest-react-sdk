...
| `formatPercent(value, options?)`   | `(value: number, options?: { decimals?: number }) => string` | Fração 0–1 como percentual pt-BR; `decimals` default `1`.  |
...
!!! tip "`decimals` é fixo: preenche e trunca"
    `formatPercent(0.5, { decimals: 3 })` é `"50,000%"`, não `"50%"`. Coluna de
    números fica alinhada, que é o caso em que a casa a mais importa — a confiança
    de um classificador a 98,7% no card de resumo e a 98,74% na tela de detalhe são
    duas leituras diferentes do mesmo número, e a precisão pertence ao call site.

!!! danger "Entrada não-finita vira `—`, não `NaN%`"
    `Intl.NumberFormat` renderiza `NaN` como `"NaN%"` e `Infinity` como `"∞%"` —
    uma divisão por zero lá em cima vira uma tela que parece bug em vez de dado
    ausente. `formatPercent` devolve `"—"`, alinhado com o `formatDurationMs` de
    `/perf`, que já respondia assim.

    O par com `percentOf` continua valendo: ele recebe **fração** (0–1), então é
...
## Valor ausente — uma resposta só

Todo campo de data de uma API é anulável (`deleted_at`, `expires_at`, metade dos `updated_at`), e todo número pode faltar. Os formatadores do SDK respondem a isso de um jeito só: **`—`**.

```ts
import { formatCurrency, formatDate, ABSENT_TEXT } from "tempest-react-sdk";

formatCurrency(saldo); // "—" quando saldo é null
formatDate(pedido.entregueEm); // "—" quando ainda não foi entregue
formatDate(pedido.entregueEm, { fallback: "pendente" }); // o texto é seu

ABSENT_TEXT; // "—", para quando a sua própria célula precisa combinar
```

Vale para `formatCurrency`, `formatBytes`, `formatCompactNumber`, `formatPercent`, `formatDate` e `formatDateTime`. Os dois que alimentam `<input>` — `formatDateForInput` e `formatDateTimeForInput` — respondem **`""`**, porque é o que um input lê como "sem valor"; um travessão chegaria como conteúdo que o usuário precisa apagar antes de digitar.

!!! danger "Antes da 0.66.0, ausente virava número plausível"
    Medido na 0.65.0, com o que chegava à tela para uma coluna que o backend deixou nula:
    ...
    Os dois zeros são o motivo de isto ser correção de dado, e não de aparência: `NaN` pelo menos **parece** quebrado, enquanto `R$ 0,00` e `0` são números que o leitor não tem como questionar — `Intl` converte `null` em zero, e ninguém a jusante distingue isso de um zero real.

    E `formatDate(null)` **derrubava a tela**: `null.getTime` é `TypeError`, então uma coluna anulável não rendia um espaço em branco, rendia um erro.

!!! tip "Ausente e inválido não são a mesma coisa"
    Valor presente que não dá para interpretar — um ISO corrompido — continua respondendo **`""`**, e isso é de propósito. Lendo um log, "ninguém preencheu" e "o que preencheram está quebrado" são achados diferentes, e o segundo é o que vale investigar. Uma célula vazia e uma com `—` param de significar a mesma coisa.
...
## String crua — `getRaw` e `setRaw`

`get`/`set` passam pelo codec **nas duas pontas**: `storage.set("tema", "dark")` grava `"dark"` — com as aspas. Isso é certo enquanto a chave é sua, e é um problema silencioso no dia em que ela não é:

```ts
import { storage } from "tempest-react-sdk";

localStorage.setItem("tema", "dark"); // o que o app já tinha, ou outra lib

storage.get("tema", "system"); // "system" — o JSON.parse falhou e caiu no fallback
storage.getRaw("tema"); // "dark"

storage.setRaw("tema", "dark"); // grava dark, sem aspas — o leitor antigo continua lendo
```

- `getRaw(key)` — devolve a string exatamente como está, ou `null` (chave ausente, storage indisponível).
- `setRaw(key, value)` — grava a string exatamente como veio.

Mesma política best-effort do par JSON: SSR-safe, nunca lança, silencia erro de quota e de modo privativo.

!!! danger "Adotar o `storage` numa chave que já existe é migração, não troca de import"
    Sem o par cru, trocar `localStorage.getItem` por `storage.get` faz duas coisas ao mesmo tempo: a leitura do valor antigo cai no `catch` e devolve o fallback, e a primeira escrita substitui o valor por uma versão entre aspas que o código antigo não reconhece mais. **Nada falha visivelmente.** Numa PWA instalada isso é preferência do usuário resetada; quando a chave guarda refresh token, é logout.

    Use `getRaw`/`setRaw` na chave que já existe, ou migre-a de propósito — lendo com `getRaw` e regravando com `set`.

!!! tip "Interop com quem não fala JSON"
    A chave `tempest-theme` do `ThemeProvider` é o exemplo dentro do próprio SDK: ela guarda `dark` puro, porque é lida também pelo script inline de `themeInitScript()`, antes de qualquer JavaScript do app rodar. Desde a v0.65.0 o `ThemeProvider` usa `storage.getRaw`/`setRaw`, então há um caminho só de acesso ao `localStorage` no SDK — e a chave continua legível por quem a lê à mão.

!!! note "Também vale para store com codec"
    `getRaw` num `compressedStorage` devolve a string comprimida com o marcador `~tgz1:`, sem descomprimir: cru quer dizer *sem codec*, não *sem compressão especificamente*.
