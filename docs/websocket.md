...
O tipo genérico (`createWebSocket<Evento>`) é uma **promessa sobre o servidor
...
export function Sala() {
    const { status } = useWebSocket<Frame>("wss://api.exemplo.com/sala", {
        schema: frameSchema,
...
!!! info "Declare o schema fora do componente"
    O `schema` é lido quando o socket abre. Um schema construído inline é um objeto
    novo a cada render — declare no módulo (ou memoize), e o que vale é o que existia
    na última abertura.

!!! check "O `pong` continua saindo"
    O ping do servidor (`{"type":"ping"}`) quase nunca casa com o schema do app —
    e ele **não** deixa de ser respondido por isso. O heartbeat é contrato do
    transporte com o servidor, não do app com o payload dele: o
    `tempest-fastapi-sdk` fecha o socket com `4408` quando o `pong` não chega,
    então a resposta é decidida do texto do frame, antes do schema. O ping em si
    não chega ao seu `onMessage` quando o schema o recusa — o que é o certo, já
    que você não o declarou.
...
- `schema` valida cada frame antes de entregar — o `pong` do heartbeat continua saindo mesmo quando o schema recusa o ping.
