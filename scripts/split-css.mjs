...
/**
 * Where the per-component sheets actually live.
 *
 * They used to sit next to the group sheets, and two pairs of names collided on a
 * case-insensitive file system: `Chat.css` against `chat.css`, `Layout.css` against
 * `layout.css`. On macOS and Windows the group — written second — won, so importing
 * the component's sheet handed back the whole family, and once a component module
 * imports its own sheet that resolution is not a byte problem any more but a wrong
 * file. A directory of its own removes the collision by construction: only
 * components are named here, and no two of them share a name.
 *
 * `styles/<Component>.css` stays, as a one-line `@import` of this file, so the
 * hand-written imports the documentation has always shown keep working.
 */
const COMPONENT_DIR = join(OUT_DIR, "component");
...
function main() {
    ...
    for (const component of byComponent.keys()) {
        ...
        if (seen !== undefined) {
            console.error(
                `split-css: "${seen}" and "${component}" are the same file on a ` +
                    "case-insensitive file system, so one component would load the other's " +
                    "CSS on macOS and Windows. Rename one of the CSS modules.",
            );
            process.exitCode = 1;
            return;
        }
        collisions.set(key, component);
    }

    for (const [component, rules] of byComponent) {
        changed += writeIfChanged(
            join(COMPONENT_DIR, `${component}.css`),
            banner + rules.join("\n") + "\n",
            check,
        )
            ? 1
        ...
        changed += writeIfChanged(
            ...
            `${banner}@import "./component/${component}.css";\n`,
            check,
        )
            ? 1
            : 0;
